...
#define FRZ_FILE_EXT ".pds"
#define FRZ_FILE_PATH "/.picodrive/"
...
static gboolean rom_get_freeze_file()
{
	...
	ptr = strrchr( current_rom_file, '/' );
	if( ptr ) ptr++; else ptr = current_rom_file;
	rom_base = strdup( ptr );

	if (current_rom_file_exists) {
		frz_file = g_strconcat( getenv( "HOME" ), FRZ_FILE_PATH, rom_base, FRZ_FILE_EXT, NULL);
		frz_exists =
...
void game_state_update()
{
	...
	m = dbus_message_new_method_call("com.jcq.picodrive.startup",
										"/com/jcq/picodrive/startup",
										"com.jcq.picodrive.startup",
										m_name);
