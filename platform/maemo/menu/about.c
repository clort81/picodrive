...
void about_dialog(GtkWindow* parent)
{
	...
	GtkWidget* logo = gtk_image_new_from_icon_name("picodrive",
		LOGO_ICON_SIZE);
	GtkWidget* label = gtk_label_new(NULL);
	gchar * label_caption = g_strdup_printf("<b>%s</b> %s",
		"PicoDrive", G_STRINGIFY(GAME_VERSION));
	...
	GtkWidget* url_label = gtk_link_button_new("http://jcqmaemo.blogspot.com/");
	GtkWidget* upstream_label = gtk_label_new("A Mega Drive emulator for Maemo.");
	GtkWidget* credits1_label = gtk_label_new("Maemo port by Javier Campo.");
	GtkWidget* credits2_label = gtk_label_new("PicoDrive originally written by notaz (http://notaz.gp2x.de).");
	...
	gtk_box_pack_start(GTK_BOX(dialog->vbox), credits1_label, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(dialog->vbox), credits2_label, FALSE, FALSE, 0);
